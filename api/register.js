...
import fs from "fs";
import formidable from "formidable";
import FormData from "form-data";          // npm i formidable form-data
import fetch from "node-fetch";             // npm i node-fetch

const GAS_URL = process.env.GAS_URL;        // set to your /exec URL in Vercel env
...
export default async function handler(req, res) {
    ...
    form.parse(req, async (err, fields, files) => {
        ...
        try {
            // Build a new FormData to forward to GAS
            const fd = new FormData();

            // forward all text fields
            for (const [k, v] of Object.entries(fields)) {
                // formidable may return arrays
                fd.append(k, Array.isArray(v) ? v[0] : v);
            }

            // forward the file under the SAME key "receipt"
            const f = files.receipt; // <-- must match <input name="receipt">
            if (f) {
                const stream = fs.createReadStream(f.filepath);
                fd.append("receipt", stream, {
                    filename: f.originalFilename || "receipt",
                    contentType: f.mimetype || "application/octet-stream",
                    knownLength: f.size,
                });
            }

            // Post to Apps Script Web App (/exec)
            const r = await fetch(GAS_URL, { method: "POST", body: fd });
            const text = await r.text(); // GAS may return JSON as text
